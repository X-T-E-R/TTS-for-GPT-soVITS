...
def send_request(endpoint, endpoint_data, text, cha_name, text_language, top_k, top_p, temperature, character_emotion,phones):
    
    phones = phones.get("phones",None)
    ...
    final_endpoint = endpoint_template.substitute(chaName=cha_name, speakText=urlencoded_text,textLanguage=text_language, topK=top_k, topP=top_p, temperature=temperature, characterEmotion=character_emotion,phones=phones)

    endpoint_data_template = Template(endpoint_data)
    filled_json_str = endpoint_data_template.substitute(chaName=cha_name, speakText=urlencoded_text,textLanguage=text_language, topK=top_k, topP=top_p, temperature=temperature, characterEmotion=character_emotion,phones=phones)
    # 解析填充后的JSON字符串
...
from text_cleaner import clean_text_inf_no_sequence

def generate_phones(text,text_language):
    phones , word2ph, norm_text = clean_text_inf_no_sequence(text, "zh")
    
    phones_str=" | ".join(phones)
    return gr.Textbox(phones_str,interactive=True)

def split_phones(phones_str):
    print(f"text changed:phone= {phones_str.split(' | ')}")
    return {"phones":phones_str.split(" | ")}



with gr.Blocks() as app:
    characters_and_emotions = gr.State({})
    phones=gr.State({"phones":[]})
    ...
    with gr.Row():
        text = gr.Textbox(value=default_text, label="输入文本",interactive=True,lines=8,scale=3)
        generate_phones_button = gr.Button("生成音素",variant="primary",scale=1)
        phones_textbox = gr.Textbox("",label="编辑音素",interactive=True,scale=3)
    ...
    with gr.Row():
        ...
        sendData.click(send_request, inputs=[endpoint, endpoint_data, text, cha_name, text_language, top_k, top_p, temperature, character_emotion, phones], outputs=[audioRecieve])
    phones_textbox.change(split_phones,inputs=[phones_textbox],outputs=[phones])
    generate_phones_button.click(generate_phones,inputs=[text,text_language],outputs=[phones_textbox])
